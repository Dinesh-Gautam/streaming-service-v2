...
const PopularMoviesBanner = ({ popularMovies }: { popularMovies: MoviesGetPopularResult[] }) => {
  const [currentIndex, setCurrentIndex] = useState(1);

  const prevIndex = currentIndex < 1 ? popularMovies.length - 1 : currentIndex - 1
  const nextIndex = currentIndex >= popularMovies.length - 1 ? 0 : currentIndex + 1;
          ...
          return (
            <div
              ...
              className={classNames(
                styles.banner,
                getPositionClassName(index, nextIndex, prevIndex, currentIndex))
              }
            >
              <Link href={getLinkHref(movie.id)}>
                <BackgroundImage id={movie.id} imgSrc={popularMovies[index].backdrop_path} />
              </Link>
              <AnimatePresence>
                {!disable && currentIndex === index && (
                  <FadeInOnMount>
                    <Bottom title={movie.title} />
                  </FadeInOnMount>
                )}
              </AnimatePresence>
            </div>
...
function getPositionClassName(index: number, nextIndex: number, prevIndex: number, currentIndex: number) {
  if (index == nextIndex)
    return styles.right
