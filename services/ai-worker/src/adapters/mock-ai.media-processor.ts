...
import {
  ...
  mockAudioPaths,
  mockImagePaths,
} from '@ai-worker/adapters/mock-data';
...
@injectable()
export class MockAIMediaProcessor
  extends EventEmitter
  implements IMediaProcessor<AIEngineOutput>
{
  private name = 'MOCK_AIMediaProcessor';
  ...
  private async runVideoAnalysis(
    ...
    const analysisResult = mockAianalysisData as any;
    logger.info(
  ...
  private async generateImages(aiData: AiVideoAnalysisResponseType) {
    ...
    try {
      const imageResult = mockImagePaths as any;
  ...
  private async processAudio(
    ...
    try {
      ...
      dubbedAudioPaths = mockAudioPaths as any;
    } catch (e: any) {
      const error = new AppError(
